...
\small
	...
	\def\cdS{\textcolor{gray}{\rmfamily\emph{cd}\texttt{S}}}
	\def\cdB{\textcolor{gray}{\rmfamily\emph{cd}\texttt{B}}}
	\def\cdum{\rmfamily\itshape\textcolor{gray}{cd}um}
	\def\cd{\textcolor{gray}{\rmfamily\emph{cd}}}
	
	\def\svarkword#1{\emph{\textrm{#1}}}



\def\varkword#1{\emph{\textrm{#1}}} % Variable keyword (argument)
\def\option#1{\textcolor{gray}{#1}}
			
\def\kword#1{\texttt{#1}}
\def\mkword#1{\texttt{#1}}
...
\end{document}
%
%\begin{minipage}{.436\textwidth}
		%
			%\begin{tabular}{p{.5cm}@{\hspace{5mm}}p{7cm}}
					%%
				%\multicolumn{2}{c}{\bfseries \emph{um}: update mode} \\
				%\hline
					%%
					%\code{FA}{ascending, starting from \reg}
					%\code{EA}{ascending, starting from \reg + 4}
					%\code{FD}{descending, starting from \reg}
					%\code{ED}{ascending, starting from \reg - 4}\hline
			%\end{tabular}
			%\vspace{.715cm}
			%
